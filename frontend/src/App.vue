  ...
  <div class="app-wrapper">
    ...
    <div class="main-container">
            ...
            <div v-if="result && !loading" class="result-content">
              ...
              <div class="metrics-grid">
                ...
                <div class="metric-item">
                  <div class="metric-icon">📉</div>
                  <div class="metric-value">
                    ¥{{ formatSalary(result.salary_range.low) }}
                  </div>
                ...
                <div class="metric-item">
                  <div class="metric-icon">📈</div>
                  <div class="metric-value">
                    ¥{{ formatSalary(result.salary_range.high) }}
                  </div>
              ...
              <div class="range-bar-section">
                <div class="range-label">
                  ...
                  <span class="range-text"
                    >{{ formatSalary(result.salary_range.low) }} \~
                    {{ formatSalary(result.salary_range.high) }}</span
                  >
                </div>
                <el-progress
      ...
      <footer class="app-footer">
        <p>
          本系统基于 GradientBoosting 机器学习模型 · 数据仅供参考 ·
          实际薪资受市场供需影响
        </p>
        <p>
          Tech Stack: Python · FastAPI · Scikit-learn · Vue 3 · Element Plus
        </p>
      </footer>
    </div>
  </div>
...
const statsCards = computed(() =>
  stats.value
    ? [
        {
          icon: "🧠",
          value: stats.value.model_type.replace("Regressor", ""),
          label: "模型算法",
        },
        {
          icon: "📦",
          value: stats.value.training_samples.toLocaleString(),
          label: "训练样本",
        },
        { icon: "🎯", value: stats.value.r2_score, label: "R² 准确率" },
        {
          icon: "🗺️",
          value: `${stats.value.supported_cities} 城市`,
          label: "覆盖范围",
        },
      ]
    : [],
